    ...
    public function listTeamachievements(Request $request)
    {
        try {
            ...
            if ($request->achievement_id == 9 || $request->achievement_id == 3 ) {
                $teamMembers->whereIn('member_id', $members)->where('user_id', $request->user_id);
            ...
            foreach ($distinctTeamMembers as $key => $value) {
                ...
                $achievements = AchievementTracker::where('user_id', $userId)->where('achievement_id', $achievement_id)->with('user')->get();
